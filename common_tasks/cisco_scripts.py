...
class CiscoScripts:
    def __init__(self, runner_class_instance):
        self.runner = runner_class_instance
    ...
    def save_configuration(self):
        ...
        :return:
        """
        pass  # TODO - device_type agnostic function
    ...
    def get_mgmt_vrf(self):
        ...
        :return:
        """
        return self.runner.get_mgmt_vrf()
    ...
    def show_interface_brief(self):
        ...
        :return:
        """
        return self.runner.show_intf_brief()

    def ping(self):
        pass

    def image_device(self, image_name):
        self.runner.priv_exec()

        # Gets the directory on the router/switch to copy to, such as flash:, bootflash:, etc.
        copy_dir = self.runner.get_copy_directory()

        sh_run_boot = '\n'.join(self.runner.get_command_output("sh run | i boot"))
        sh_boot = '\n'.join(self.runner.get_command_output("sh boot"))
        ...
        self.runner.global_config()

        self.runner.send("no boot {} \r".format(boot_var))
        self.runner.send("boot {} {}{} \r".format(boot_var, copy_dir, image_name))

        self.runner.priv_exec()
        self.runner.save_changes()

        self.runner.crt.Dialog.MessageBox("Please MANUALLY double check boot variable, and then reload.")

    def get_copy_directory(self, runner):
        dir_output = '\n'.join(self.runner.get_command_output("dir | i :/"))
    ...
    def copy_from_ftp_server(self, hostname, username, password, title="Please select an image file", filter="Image files (*.bin)|*.bin||"):
        ...
        image_full_path = self.runner.crt.Dialog.FileOpenDialog(title, filter)
        ...
        try:
            ...
            self.runner.crt.Dialog.MessageBox("Exception message: {}".format(e))
    ...
    def copy_ftp(self, runner, hostname, source_filename, username="", password="", vrf=None, destination_dir=None,
        ...
        if vrf is None:
            self.runner.current_tab.Screen.Send("copy ftp://{}:{}@{} {} \r".format(username,
                                                                                   password,
                                                                                   hostname,
                                                                                   destination_dir))

        else:
            self.runner.current_tab.Screen.Send("copy ftp://{}:{}@{} {} vrf {} \r".format(username,
                                                                                          password,
                                                                                          hostname,
                                                                                          destination_dir,
                                                                                          vrf))
        ...
        while True:
            result = self.runner.current_tab.Screen.WaitForStrings(["Address",
                                                                     "filename",
                                                                     "Destination",
                                                                     "write",
                                                                     "OK",
            ...
            if result == 1:
                self.runner.send("\r")
            if result == 2:
                self.runner.current_tab.Screen.Send(source_filename + "\r")
            if result == 3:
                self.runner.send("\r")
            if result == 4:
                self.runner.send("\r")
            ...
            if result == 6:
                ...
                self.runner.current_tab.Screen.WaitForString("OK", copy_timeout)
            if result == 0:
                # TODO - timeout error
                break
